...
import 'package:solarsense/src/features/core/controllers/playground_controller.dart';
...
class PickedEquipmentCardList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String panelID = splitRecommendation[0]
        .substring(splitRecommendation[0].indexOf(':') + 2);
    print("panelID = ...$panelID...");
    final String inverterID = splitRecommendation[1]
        .substring(splitRecommendation[1].indexOf(':') + 2);
    print("inverterID = ...$inverterID...");
    final String noOfPanels = splitRecommendation[2]
        .substring(splitRecommendation[2].indexOf(':') + 2);
    print("noOfPanels = $noOfPanels");

    // final playgroundStats = PlaygroundStatsModel(
    //     email: email,
    ...
    // );
    //
    // PlaygroundController.instance.createPlaygroundStats(playgroundStats);
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) => GestureDetector(
                  ...
                  child: equipmentList[index] is PanelModel
                      ? PickedPanelCard(
                          product: equipmentList[index] as PanelModel,
                          panelCount: noOfPanels,
                          txtTheme: txtTheme)
                      : PickedInverterCard(
                          product: equipmentList[index] as InverterModel,
                          txtTheme: txtTheme),
                ),
              ),
            );
